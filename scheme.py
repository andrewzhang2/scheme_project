...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if isinstance(operator, MacroProcedure):
            expression = operator.apply_macro(rest, env)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 9
        lambda_expression = Pair(target.rest, expressions.rest)
        env.define(target.first, do_lambda_form(lambda_expression, env))
        return target.first
        # END PROBLEM 9
    else:
...
def make_let_frame(bindings, env):
    ...
    while bindings!= nil:
        ...
        args = Pair(scheme_eval(bindings.first.rest.first, env), args)
        bindings = bindings.rest
    check_formals(formals)
    child_frame = Frame.make_child_frame(env, formals, args)
    return child_frame
    # END PROBLEM 14
